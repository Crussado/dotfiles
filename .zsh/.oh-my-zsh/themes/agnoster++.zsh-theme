...
# Hook que se ejecuta antes de un comando
preexec() {
...
prompt_status() {
  ...
  [[ $RETVAL -ne 0 ]] && symbols+="%{%F{54}%}$RETVAL %{%F{54}%}✘"
  [[ $UID -eq 0 ]] && symbols+="%{%F{yellow}%}⚡"
  [[ $(jobs -l | wc -l) -gt 0 ]] && symbols+="%{%F{cyan}%}⚙"

  [[ -n "$symbols" ]] && prompt_segment_right 101 "default" "$symbols "
}

prompt_time() {
  if [[ $LAST_COMMAND_DURATION -gt 0 ]]; then
    cmd_time="${LAST_COMMAND_DURATION}s"
    prompt_segment_right 3 0 "$cmd_time "
  fi
}
...
build_prompt() {
  prompt_virtualenv
  prompt_aws
  [[ -z "$ZSH_CONTEXT_PROMT" || "$ZSH_CONTEXT_PROMT" = true ]] && prompt_context
  prompt_dir
  prompt_git
  prompt_bzr
  prompt_hg
